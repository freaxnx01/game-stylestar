...
const sp = (x, y, s, c = '#fff') => `<path d='M${x} ${y - s} L${x + s * .35} ${y - s * .35} L${x + s} ${y} L${x + s * .35} ${y + s * .35} L${x} ${y + s} L${x - s * .35} ${y + s * .35} L${x - s} ${y} L${x - s * .35} ${y - s * .35} Z' fill='${c}'/>`;
const dot = (x, y, r, c = '#fff') => `<circle cx='${x}' cy='${y}' r='${r}' fill='${c}'/>`;

export const SKINS = ['#F8CEAA', '#DCA478', '#9C6B43'];

export const dollUri = (c) => uri(`
...
const capSleeve = c => `<path d='M102 160 Q150 144 198 160 L207 212 Q197 222 187 214 L187 258 Q150 272 113 258 L113 214 Q103 222 93 212 Z' fill='${c}'/>`;
const tank = c => `<path d='M112 164 Q150 152 188 164 L187 258 Q150 272 113 258 Z' fill='${c}'/>`;
const sleeves = c => `<path d='M97 168 L88 296' stroke='${c}' stroke-width='21' stroke-linecap='round' fill='none'/><path d='M203 168 L212 296' stroke='${c}' stroke-width='21' stroke-linecap='round' fill='none'/>`;
const hip = c => `<path d='M116 250 Q150 263 184 250 L187 298 Q150 312 113 298 Z' fill='${c}'/>`;
const legs = (c, y, w) => `<path d='M133 292 L130 ${y}' stroke='${c}' stroke-width='${w}' stroke-linecap='round' fill='none'/><path d='M167 292 L170 ${y}' stroke='${c}' stroke-width='${w}' stroke-linecap='round' fill='none'/>`;

const TVB = { top: '80 130 145 165', bottom: '85 235 130 320', dress: '55 135 190 430', shoes: '95 500 110 100', hair: '55 0 190 250' };

const fl = (x, y) => [0, 72, 144, 216, 288].map(a => dot(x + 5.5 * Math.cos(a * Math.PI / 180), y + 5.5 * Math.sin(a * Math.PI / 180), 4, '#FBB6D9')).join('') + dot(x, y, 3.2, '#FFCB47');
...
].map(h => ({ ...h, backImg: uri(h.back), frontImg: uri(h.front), thumb: uri(h.back + `<ellipse cx='150' cy='88' rx='50' ry='54' fill='#F3D9C0'/>` + h.front, TVB.hair) }));
